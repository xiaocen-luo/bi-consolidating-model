    ...
    def __getitem__(self, idx):
        ...
        if not self.is_test:
            ...
            for triple in ins_json_data['triple_list']:
                rel_index = self.rel2id[triple[1]]
            ...
            if s2ro_map:
                ...
                dist_inputs = np.zeros((text_len, text_len), dtype=int)
                ...
                for s in s2ro_map:
                    ...
                    for ro in s2ro_map.get((sub_head, sub_tail), []):
                        ...
                        triple_matrix[relation][sub_head][obj_head] = self.tag2id['B-B']
                        triple_matrix[relation][sub_head][obj_tail] = self.tag2id['B-E']
                        triple_matrix[relation][sub_tail][obj_tail] = self.tag2id['E-E']
                
                return token_ids, masks, loss_masks, text_len, triple_matrix, ins_json_data['triple_list'], tokens, dist_inputs
            else:
                return None
...
def re_collate_fn(batch):
    ...
    rel_num = 24  # if use WebNLG_star, modify 24 to 171   216  29  12
    
    token_ids, masks, loss_masks, text_len, triple_matrix, triples, tokens, dist_inputs = zip(*batch)
    ...
    batch_dist_inputs = torch.LongTensor(cur_batch_len, 1, max_text_len, max_text_len).zero_()
    ...
    for i in range(cur_batch_len):
        ...
        batch_dist_inputs[i, 0, :text_len[i], :text_len[i]].copy_(torch.from_numpy(dist_inputs[i]))
    ...
    return {'token_ids': batch_token_ids,
            ...
            'triples': triples,
            'tokens': tokens,
            'dist_inputs': batch_dist_inputs}

def get_loader(config, prefix, is_test=False, num_workers=4, collate_fn=re_collate_fn):
